...
class Migration(DataMigration):
	...
	for organization in orm.Organization.objects.all():
	    ...
	    translations = orm.OrganizationTranslation.objects.filter(organization=organization)
	    if translations:
	        try:
		    translation = translations.get(
	                language=settings.LANGUAGE_CODE)
		except orm.OrganizationTranslation.DoesNotExist:
	...
	for project in orm.Project.objects.all():
	    # Default slug, should be '' after previous migration
	    slug = project.slug
	    translations = orm.ProjectTranslation.objects.filter(project=project)
	    if translations:
	        try:
		    translation = translations.get(
	                language=settings.LANGUAGE_CODE)
		except orm.ProjectTranslation.DoesNotExist:
	            translation = translations[0]
