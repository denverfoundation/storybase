...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # After adding the status field to the Project and Organization
        # models, the status will be set to the default value of 'draft'.
        # Since there was no publication status originally, we want to set
        # the status to 'published'
        orm.Organization.objects.all().update(status='published')
        orm.Project.objects.all().update(status='published')

    def backwards(self, orm):
        # Set the status back to the default, 'draft'
        orm.Organization.objects.all().update(status='draft')
        orm.Project.objects.all().update(status='draft')
