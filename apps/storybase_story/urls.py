...
#        name='story_search'),
    ...
    url(r'build/(?P<story_id>[0-9a-f]{32,32})/(?P<stage>data|review)/$', StoryBuilderView.as_view(), name='story_builder'),
    url(r'build/(?P<story_id>[0-9a-f]{32,32})/(?P<stage>share)/(?P<substage>legal|tagging|publish)/$', StoryBuilderView.as_view(), name='story_builder'),
