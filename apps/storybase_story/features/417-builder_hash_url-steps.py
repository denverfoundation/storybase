...
def open_story_in_builder_hash_step_hash(step, title, wkflw_step):
    story = Story.objects.get(storytranslation__title=title)
...
@step(u'Given the user opens the Story "([^"]*)" in the story builder with a hashed story ID in the "([^"]*)" workflow step')
...
@step(u'Then the user should be redirected to the story builder for the Story "([^"]*)" in the "([^"]*)" workflow step without a hashed story ID in the URL')
def has_story_builder_path_no_hash_step(step, title, wkflw_step):
    has_story_builder_path_step_no_hash(step, title, wkflw_step)
