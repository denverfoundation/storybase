...
class Migration(SchemaMigration):
    ...
    depends_on = (
        ("storybase_geo", "0001_initial.py"),
    )

    def forwards(self, orm):
        
        # Adding M2M table for field locations on 'Story'
        db.create_table('storybase_story_story_locations', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('story', models.ForeignKey(orm['storybase_story.story'], null=False)),
            ('location', models.ForeignKey(orm['storybase_geo.location'], null=False))
        ))
        db.create_unique('storybase_story_story_locations', ['story_id', 'location_id'])


    def backwards(self, orm):
        
        # Removing M2M table for field locations on 'Story'
        db.delete_table('storybase_story_story_locations')
