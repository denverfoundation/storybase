# -*- coding: utf-8 -*-
from __future__ import unicode_literals

from django.db import migrations, models
import uuid

from apps.utils import transform, to_uuid, to_char32

class Migration(migrations.Migration):

    dependencies = [
        ('storybase_messaging', '0002_storynotification_story'),
    ]

    operations = [
        migrations.AlterField(
            model_name='sitecontactmessage',
            name='email',
            field=models.EmailField(max_length=254, verbose_name='Your Email'),
        ),

        # SystemMessageTranslation.translation_id
        migrations.AddField(
            model_name='systemmessagetranslation',
            name='tmp_translation_id',
            field=models.UUIDField(null=True, default=None),
        ),
        migrations.RunPython(
            transform(to_uuid, 'storybase_messaging', 'SystemMessageTranslation', 'translation_id'),
            transform(to_char32, 'storybase_messaging', 'SystemMessageTranslation', 'translation_id'),
        ),
        migrations.RemoveField(
            model_name='systemmessagetranslation',
            name='translation_id',
        ),
        migrations.RenameField(
            model_name='systemmessagetranslation',
            old_name='tmp_translation_id',
            new_name='translation_id',
        ),
        migrations.AlterField(
            model_name='systemmessagetranslation',
            name='translation_id',
            field=models.UUIDField(null=False, default=uuid.uuid4),
        ),
    ]
